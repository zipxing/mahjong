    ...
    execute: function () {
      ...
      _export("EliminationManager", EliminationManager = class EliminationManager {
        ...
        hasValidMoves(boardData) {
          ...
          for (let row = 0; row < boardSize; row++) {
            for (let col = 0; col < boardSize; col++) {
              ...
              const connectedBlocks = this.findConnectedBlocksWithVisited(boardData, row, col, visited); // 如果找到2个或以上的连通方块，说明还有有效移动
        ...
        highlightConnectedBlocks(boardData, startRow, startCol, highlightCallback) {
          const connectedBlocks = this.findConnectedBlocks(boardData, startRow, startCol); // 只有2个或以上的方块才进行高亮
        ...
        getConnectedRegionsStats(boardData) {
          ...
          const blockTypeDistribution = {}; // 遍历整个棋盘
          ...
          for (let row = 0; row < boardSize; row++) {
            for (let col = 0; col < boardSize; col++) {
              ...
              const blockType = boardData[row][col].type; // 查找连通区域

              const connectedBlocks = this.findConnectedBlocksWithVisited(boardData, row, col, visited);
