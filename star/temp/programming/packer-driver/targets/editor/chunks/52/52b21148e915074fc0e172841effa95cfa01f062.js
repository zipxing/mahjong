    ...
    execute: function () {
      ...
      _export("BoardManager", BoardManager = class BoardManager {
        ...
        adaptToScreenSize(defaultBlockSize, defaultBlockSpacing) {
          console.log(`🔧 使用固定配置，确保严格10×10方块`); // 修复间距问题：使用合理的方块尺寸和间距比例

          this.blockSize = 40; // 缩小方块到40px

          this.blockSpacing = 6; // 增大间距到6px (15%的方块尺寸)

          const totalSize = this.boardSize * this.blockSize + (this.boardSize - 1) * this.blockSpacing; // 10×40 + 9×6 = 400 + 54 = 454px
        ...
        generateBoard(blockManager) {
          ...
          for (let row = 0; row < 10; row++) {
            for (let col = 0; col < 10; col++) {
              const blockType = Math.floor(Math.random() * blockManager.getBlockTypeCount()); // 添加随机数调试
          ...
          console.log(`   - GameBoard ContentSize: ${(_this$gameBoardNode$g = this.gameBoardNode.getComponent(UITransform)) == null ? void 0 : _this$gameBoardNode$g.contentSize.width}×${(_this$gameBoardNode$g2 = this.gameBoardNode.getComponent(UITransform)) == null ? void 0 : _this$gameBoardNode$g2.contentSize.height}`); // 检查前几个子节点
        ...
        gridToLocal(row, col) {
          // 计算棋盘的总尺寸
          const totalWidth = this.boardSize * this.blockSize + (this.boardSize - 1) * this.blockSpacing;
          const totalHeight = this.boardSize * this.blockSize + (this.boardSize - 1) * this.blockSpacing; // 计算起始偏移（让棋盘居中）

          const startX = -totalWidth / 2 + this.blockSize / 2;
          const startY = totalHeight / 2 - this.blockSize / 2; // 计算具体位置

          const x = startX + col * (this.blockSize + this.blockSpacing);
          const y = startY - row * (this.blockSize + this.blockSpacing); // 添加详细调试信息（前几个方块）
